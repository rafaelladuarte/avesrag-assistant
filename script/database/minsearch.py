    ...
    def search(self, query, filter_dict={}, boost_dict={}, num_results=10):
        ...
        for field, value in filter_dict.items():
            if field in self.keyword_fields:
                ...
                mask = self.keyword_df[field].apply(
                    lambda x: any(v in x for v in value) if isinstance(x, list)
                    else x in value if isinstance(value, list)
                    else value in x if isinstance(x, list)
                    else x == value
                )
        ...
        top_indices = np.argpartition(final_score, -num_results)[-num_results:]
        top_indices = top_indices[np.argsort(-final_score[top_indices])]
